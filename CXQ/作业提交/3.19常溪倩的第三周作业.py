...
2.len():计算列表长度
3.列表切片，使用索引进行切片，用法:列表名[起始长度:终止长度:步长],起始可以取到值，终止取不到值，左闭右开区间；在步长为正数，起始值必须小于终止值
4.列表的加法与乘法
...
9.设置摆放家具的程序.min():计算最小值，返回列表中最小的元素
...
15.count():计算元素在列表的个数
16.sort():对列表进行排序，根据元素的编码进行排序，默认升序排列(reverse=False),reverse=True 降序排列
17.append():可以向列表中添加元素，添加到末尾位置
18.insert():根据索引插入元素
19.extend():将两个列表按照顺序进行合并元素
20.pop():删除元素，默认删除最后一个，可以根据索引删除对应元素
21.remove()移除元素，直接将对应的元素移除，有多个重复元素，默认删除第一个
22.reverse():翻转列表
23.copy():克隆一个列表，新与旧的内存地址不一样，不是同一个列表，元素一样del删除关键字
24.clear():清空列表
...
18.isdight():判断是否为纯数字，是纯数字结果为True，否则为False
19.isalnum():判断字符串是否字符，没有字符为True，有符号为False
20.isupper():判断是否全部字母为大写
21.islower():判断是否全部字母为小写
"""

"""
2.设置闰年判断程序.py 完成学生信息记录
...
"""
stu_list = ["202115050201", "周杰伦", "男", "人工智能2班"], ["202115050202", "迪丽热巴", "女", "人工智能2班"]
while True:
    print("""
-----------------------------    
    1.添加新的学生信息
