...
fa.close()
"""a+:追加写模式，打开文本，文件不存在会创建，不会清空文本，会接着文本的末尾写入，只会在末尾写入与光标无关"""
faj = open("临江仙.txt", "a+", encoding="utf-8")
print(faj.readable())    # 可读
print(faj.writable())    # 可写
faj.write("滚滚长江东逝水，浪花淘尽，是非成败转有空\n")
# 读取数据
faj.seek(0)
text = faj.read()
print(text)
...
# print(fab.read())
fab.close()
# 练习创建一个文件stus.txt,每一行数据如下
# 学号：123456789123-姓名：张三-性别：男-班级：智能1-成绩：80
# 使用a+模式，键盘写入一些数据；大概10条
# 读取数据进行筛选，成绩低于60分还以上边格式补考文件
