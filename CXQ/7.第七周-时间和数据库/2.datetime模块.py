...
# ---------------date类------------------
# 1.使用date类构造一个日期对象
dt = datetime.date(year=2022, month=4, day=1)
print(dt, type(dt))
# 2.today():获取当前的日期
td = datetime.date.today()
print(td)
# fromtimestamp(时间戳):将时间戳转化成日期
print(time.time())
...
print(td.toordinal())        # fromordinal(),逆推算，通过日期推算出公园1-1-1到今天有多少天
...
# -------------------time类--------------------------
# 1.构造时间
tm = datetime.time(hour=10, minute=11, second=12)
print(tm)
# 2.isoformat():返回标准格式化时间
t2 = tm.isoformat()
print(t2)
# 3.strftime():格式化时间
