...
# 2.设置闰年判断程序.py 使用连接对象创建游标(用来执行sql语句, 读取结果数据)
...
# 4. 插入数据
insert_sql = """insert into stu(id, name, phone) values (20020410, "孔丘", 1588888882)"""
# cursor.execute(insert_sql)
# 查询数据
select_sql = """select * from stu where name='孔丘';"""
cursor.execute(select_sql)
# 通过游标取出数据, 游标的数据读取一条少一条
res = cursor.fetchone()     # fetchone(): 从查询结果中读取一条数据, 以元组形式返回
print(res)
# res2 = cursor.fetchall()    # fetchall(): 从结果读取所有的数据, 列表中放入元组
# print(res2)
