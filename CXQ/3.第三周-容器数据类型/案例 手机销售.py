"""
手机销售
    1.多个品牌的手机，华为，苹果，oppo，小米，锤子...
    2.功能
        1. 添加功能
            1) 查询是否有该手机, 有则直接增加库存
            2) 没有品牌则添加键值对(品牌及库存)
        2.设置闰年判断程序.py 查询手机品牌和库存
            1) 单个查询, 手机品牌及库存
            2) 查询所有, 将所有品牌及库存输出
        3.手机出售, 选择一个品牌,选择购买的数量, 扣除库存, 购买成功
            1) 先查看品牌, 选择购买的品牌
            2) 输入购买数量, 判断存库是否够用
        ...
        4.当手机没库存, 删除这个手机品牌, 删除库存
            ...
            2) 可以手动修改库存数量
"""
phone_count = {"华为": 800,"iphone": 900, "小米": 998}
while True:
    print("""
------------------------------
    欢迎使用手机销售商城
...
-------------------------------    
    ...
    if select_num == "1":
        ...
        # phone_count.get(logo,""):当logo存在，返回库存，不存在返回""
        print("请输入库存数量")
        num = input("新增手机数量(整数): ")
        ...
        if logo in phone_count.keys():
            ...
            print(f"| 手机已经入库,{logo}手机库存: {phone_count[logo]}, 新增:{num}")
    elif select_num == "2":
            print("查询手机信息")
            if phone_count:
                print("""
